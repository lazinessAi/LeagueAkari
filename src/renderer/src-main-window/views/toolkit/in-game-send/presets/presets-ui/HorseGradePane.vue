    ...
    <div class="pt-2">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <div class="flex items-center gap-1.5">
            <span class="text-xs text-black/60 dark:text-white/60">{{ tAi('shortcut') }}</span>
            <ShortcutSelector
    ...
    <SettingsRow class="mt-3" :label-width="180" align="center" no-x-padding control-full-line>
      ...
      <div class="flex w-full items-center gap-2">
        <NInput
          ...
          size="small"
          :placeholder="tAi('manual.placeholder')"
...
const sendButtonText = computed(() => {
  if (gamePhase.value === 'in-game') {
    return tAi('sendToGame')
  }

  return tAi('sendToChat')
})
...
function targetDescription(target: { id: HorseGradeTargetId; description: string }) {
  const count = horseGrade.getTargetPlayers(target.id).length
  return `${target.description} · ${tAi('teamsCount', { count })}`
}
...
function manualPlayerStatusText() {
  ...
  switch (entry.status) {
    case 'fetching':
      return tAi('playerStatus.fetching')
    case 'analyzing':
      return tAi('playerStatus.analyzing')
    case 'no-data':
      return tAi('playerStatus.noData')
    case 'error':
      return tAi('playerStatus.error', { message: entry.errorMessage })
    default:
      return ''
  }
}
...
async function handleSendTarget(target: HorseGradeTargetId) {
  const sent = await horseGrade.send(target)
